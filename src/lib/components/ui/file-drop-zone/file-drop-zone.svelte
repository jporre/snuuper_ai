<script lang="ts" module>
  ...
  export type FileRejectedReason = 'Maximum file size exceeded' | 'File type not allowed' | 'Maximum files uploaded' | 'too-many-files' | 'file-invalid-type' | 'file-invalid-type' | 'file-too-large'
...
<script lang="ts">
  ...
  const shouldAcceptFile = (file: File, fileNumber: number): FileRejectedReason | undefined => {
    ...
    if (!isAcceptable) return 'file-invalid-type'
    return undefined
  }
  const canUploadFiles = $derived(!disabled && !uploading && !(maxFiles !== undefined && fileCount !== undefined && fileCount >= maxFiles))
</script>
<label
  ...
  class={cn('border-border hover:bg-accent flex h-48 w-full place-items-center justify-center rounded-lg border-2 border-dashed p-6 transition-all hover:cursor-pointer aria-disabled:opacity-50 aria-disabled:hover:cursor-not-allowed', className)}>
  {#if children}
    {@render children()}
  {:else}
    <div class="flex flex-col justify-center gap-2 place-items-center">
      <div class="flex justify-center border border-dashed rounded-full border-border text-muted-foreground size-14 place-items-center">
        <Upload class="size-7" />
      </div>
      <div class="flex flex-col gap-0.5 text-center">
        <span class="font-medium text-muted-foreground"> Puede arrastrar los archivos y hacer click </span>
        {#if maxFiles || maxFileSize}
          <span class="text-sm text-muted-foreground/75">
            {#if maxFiles}
              <span>Puede Subir un Màximo de {maxFiles} Archivos</span>
            {/if}
            {#if maxFiles && maxFileSize}
              <span>(con un máximo de {displaySize(maxFileSize)} cada uno)</span>
            {/if}
            {#if maxFileSize && !maxFiles}
              <span>Peso máximo permitido {displaySize(maxFileSize)}</span>
            {/if}
          </span>
        {/if}
      </div>
  ...
  {/if}
  <input {...rest} disabled={!canUploadFiles} {id} {accept} multiple={maxFiles === undefined || maxFiles - (fileCount ?? 0) > 1} type="file" onchange={change} class="hidden" />
</label>
